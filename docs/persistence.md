...
`TournamentDatabaseInitializer` aplica migraciones al iniciar la API y reutiliza el seed versionado cuando la tabla Teams está vacía. Puede desactivarse con `Database:ApplyMigrations=false`. `SqlServerTournamentTests` comprueba estos componentes contra una base SQL Server exclusiva de QA; consulte `procesos-qa.md`.
